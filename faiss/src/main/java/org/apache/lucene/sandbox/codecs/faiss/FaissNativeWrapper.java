...
final class FaissNativeWrapper {
  ...
  private static MethodHandle getHandle(String functionName, FunctionDescriptor descriptor) {
    // Java 21 (JEP 442 preview) uses libraryLookup(), Java 22+ (JEP 454 final) uses loaderLookup()
    // Use runtime detection to support both
    SymbolLookup lookup;
    ...
    if (javaVersion >= 22) {
      ...
      // Java 21 preview API - use reflection since libraryLookup() doesn't exist in Java 22+
      try {
        java.lang.foreign.Arena arena = java.lang.foreign.Arena.global();
        java.lang.reflect.Method libraryLookupMethod =
            SymbolLookup.class.getMethod("libraryLookup", String.class, java.lang.foreign.Arena.class);
        lookup = (SymbolLookup) libraryLookupMethod.invoke(null, FaissLibrary.NAME, arena);
      } catch (java.lang.reflect.InvocationTargetException
          | IllegalAccessException
          | NoSuchMethodException e) {
        throw new RuntimeException("Failed to create SymbolLookup for Java 21", e);
      }
    }
  ...
  private final MethodHandle faiss_Index_ntotal$MH =
      getHandle("faiss_Index_ntotal", FunctionDescriptor.of(JAVA_LONG, ADDRESS));

  long faiss_Index_ntotal(MemorySegment indexPointer) {
    try {
      return (long) faiss_Index_ntotal$MH.invokeExact(indexPointer);
    } catch (RuntimeException | Error e) {
      throw e;
    } catch (Throwable t) {
      throw new AssertionError(t);
